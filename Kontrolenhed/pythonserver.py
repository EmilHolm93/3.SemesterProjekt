from flask import Flask, request, jsonify, render_template, redirect, url_for, copy_current_request_context, session
...
# To store hashmap data
@app.before_request
def initialize_chip_hash():
    if "chip_hash" not in session:
        session["chip_hash"] = {1: 0, 5: 0, 10: 0, 20: 0}
...
def login():
    ...
    if user_id:
        ...
        if int(current_user.id) == 5:
            next_url = request.args.get('next', url_for('maintenance'))  # Redirect to maintenance
            return jsonify({"status": "success", "message": "maintenance", "next": next_url}), 200
...
def maintenance():
    ...
    chip_hash = session.get("chip_hash")
    chip_1_count = chip_hash.get(1, 0)
    chip_5_count = chip_hash.get(5, 0)
    chip_10_count = chip_hash.get(10, 0)
    chip_20_count = chip_hash.get(20, 0)

    print(f"User is authenticated: {current_user.is_authenticated}")
    return render_template("maintenance.html", current_user=current_user, chip_1_count=chip_1_count, chip_5_count=chip_5_count, chip_10_count=chip_10_count, chip_20_count=chip_20_count)
...
def deposit():
    ...
    if request.method == "POST":
        ...
        if valid_deposits:
            ...
            chip_hash_map = {int(k): int(v) for k, v in session.get("chip_hash", {1: 0, 5: 0, 10: 0, 20: 0}).items()}

            new_chip_hash = hash_map_convert(valid_deposits)

            # Merge the two dictionaries (add counts) - make sure both are integers
            for denom, count in new_chip_hash.items():
                chip_hash_map[denom] = chip_hash_map.get(denom, 0) + count

            session["chip_hash"] = chip_hash_map

            print("Updated chip_hash map after deposit:", chip_hash_map)

            insertion_sorting(valid_deposits)
